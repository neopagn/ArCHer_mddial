...
def process_conversation(lines):
    pairs = []
    # Bắt đầu context với vai trò của Doctor để mô hình biết nó là bác sĩ
    current_context = "Doctor-Patient Conversation:\n" # Bác sĩ bắt đầu

    for i, line in enumerate(lines):
        if "Based on your symptoms" in line:
            # Đây là dòng chẩn đoán cuối cùng
            # observation: Context hiện tại của hội thoại (gồm cả lượt bệnh nhân cuối cùng)
            # action: Dòng chẩn đoán đầy đủ, bắt đầu bằng "Doctor: "
            pairs.append({
                'observation': current_context.strip(),
                'action': f"Doctor: {line.strip()}" # Đảm bảo hành động là từ góc nhìn của Doctor
            })
            break # Kết thúc vòng lặp cho hội thoại này

        # Xử lý các dòng hỏi và trả lời triệu chứng
        # Dòng trong "lines" có định dạng "Question? Answer" (tức là đã ghép Doctor và Patient)
        # Chúng ta cần tách chúng ra để tạo cặp observation-action đúng.
        # Ví dụ: "Do you experience Cough? Yes" -> Action của Doctor: "Do you experience Cough?"
        #                                      -> Context cập nhật: "Doctor: Do you experience Cough?\nPatient: Yes"
        ...
        # Observation: Context trước khi Doctor hỏi câu hỏi này.
        # Action: Câu hỏi của Doctor.
        pairs.append({
            'observation': current_context.strip(),
            'action': f"Doctor: {question}?" # Hành động là câu hỏi của Doctor
        })

        # Cập nhật context cho lượt tiếp theo.
        # current_context sẽ chứa toàn bộ cuộc hội thoại đã diễn ra.
...
def main():
    # Configuration
    config = {
        'data_path': '/kaggle/input/mddial-json/Mddial.json',
